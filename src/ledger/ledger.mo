...
import tx_processor "tx_processor";
...
module {
  ...
  public type ProcessingError = tx_processor.ProcessingError;
  ...
  public class Ledger(initialAggregators : [Principal]) = self {
    ...
    public func virtualAccountState_(oid : OwnerId, vid: VirtualAccountId): Result<VirtualAccountState, { #UnknownVirtualAccount; #DeletedVirtualAccount; }> =
    ...
    func processBatch_(source: {#agg : Nat; #direct}, batch: Batch): [Result<(), ProcessingError>] {
      ...
      for (i in batch.keys()) {
        let res = tx_processor.processTx(self, batch[i]);
        switch (res) {
    ...
    public func processBatch(aggId: Nat, batch: Batch): () = ignore processBatch_(#agg(aggId), batch);
    ...
    public let owners : RBTree.RBTree<Principal, OwnerId> = RBTree.RBTree<Principal, OwnerId>(Principal.compare);
