...
res0;
output("./performance_tests/cycle_stats.txt", stringify("[AGG] submit simple Tx: ", res0[0], "\n"));
...
assert batch[0] != null;
output("./performance_tests/cycle_stats.txt", stringify("[AGG] submit + auto-enqueue simple Tx: ", res1[0], "\n"));

let heavy_tx = call canister.generateHeavyTx(0);
let res2 = call canister.profileSubmit(heavy_tx);
res2;
output("./performance_tests/cycle_stats.txt", stringify("[AGG] submit heavy tx: ", res2[0], "\n"));
...
let batch = call canister.getNextBatch();
assert batch[0] != null;
output("./performance_tests/cycle_stats.txt", stringify("[AGG] approve + equeue simple Tx: ", res3[0], "\n"));
...
res5;
output("./performance_tests/cycle_stats.txt", stringify("[AGG] reject simple Tx: ", res5[0], "\n"));
...
res6;
output("./performance_tests/cycle_stats.txt", stringify("[AGG] prepare big batch: ", res6[0], "\n"));
