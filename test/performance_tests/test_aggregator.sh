...
res0;
output("cycle_stats.txt", stringify("[AGG] submit simple Tx: ", res0[0], "\n"));
...
let batch = call canister.getNextBatch();
assert batch[0] != null;
output("cycle_stats.txt", stringify("[AGG] approve + equeue simple Tx: ", res3[0], "\n"));
...
res5;
output("cycle_stats.txt", stringify("[AGG] reject simple Tx: ", res5[0], "\n"));
...
res6;
output("cycle_stats.txt", stringify("[AGG] prepare big batch: ", res6[0], "\n"));
